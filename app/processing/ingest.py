...
def read_csv_chunks(
    ...
    logger.info(
        "Starting CSV chunked read",
        extra={"csv_path": str(csv_path), "chunk_size": chunk_size}
    )
    ...
    for chunk_num, chunk_df in enumerate(
        ...
        logger.debug(
            ...
            extra={
                ...
                "rows": len(chunk_df),
                "depth_range": (
                    float(chunk_df.iloc[:, 0].min()),
                    float(chunk_df.iloc[:, 0].max())
                )
            }
        )
        yield chunk_df
...
async def process_chunk_to_frames(
    ...
    if len(pixel_cols) != source_width:
        raise ValueError(
            f"Expected {source_width} pixel columns, got {len(pixel_cols)}"
        )
    ...
    logger.info(
        "Processed chunk",
        extra={
            "rows_processed": len(frames),
            "rows_failed": len(chunk_df) - len(frames)
        }
    )
